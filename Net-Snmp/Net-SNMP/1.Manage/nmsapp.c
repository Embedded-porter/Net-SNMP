...
static int read_nmsapp_conf()
{
	...
	if (NULL != (pf = fopen(PEER_FILE, "r")))
	{
		...
		while (fgets(acLine, sizeof(acLine) - 1, pf))
		{
			...
			if (NULL == strstr(acLine, LEFT_SQUARE_BRACKET) && NULL == strstr(acLine, EQUAL_SIGN))
			{
			...
			// 解析左括号
			if (NULL != strstr(acLine, LEFT_SQUARE_BRACKET) && SUCCESS == parser_delim(acLine, LEFT_SQUARE_BRACKET, acT, acV))
			{
...
static void save_peer_oids(T_List *pList, char *pcOid)
{
	...
	if (NULL != pList && NULL != pcOid && NULL != pNMSOid)
	{
		...
		// 判断最后两位是否为.0
		if ('.' == ptmp[strlen(ptmp) - 2] && '0' == ptmp[strlen(ptmp) - 1])
		{
...
static int _init_send_oids(void *data, void *ctx)
{
	...
	{
		if (NULL == pHostNode->ptCurrentOid && pHostNode->tCounter.counter++ >= pHostNode->tCounter.frequence) // 当未发送完成时，不进行初始化
		{
...
static int _send_request(void *data, void *ctx)
{
	...
	struct snmp_pdu *pdu;
	if (NULL != pNMSHost && NULL != pNMSHost->pSess && NULL != pNMSHost->ptCurrentOid)
	{
		// 取当前待发送的OID
...
int main(void)
{
	if (NULL == init_NMS_list()) // 初始化主机列表
	{
		return 1;
	}

	if (check_nmsapp_running()) // 检测是否正在运行,提高调用flock锁来控制系统只有一个nmsapp进程启动
	{
	...
	while (running)
	{
		...
		init_send_oids(); // 当监控周期到并且当前没有待发送的OID时,则置当前发送的OID为子链表头节点
		// 打印到文件fprintf(f, ".%s(%lu)", tp->label, tp->subid);
		asynch_send();	// 异步发送函数
		wait_request(); // 等待所有已经发送的PDU请求包的响应,并且读取PDU的信息
		sleep(1);		// usleep
	}
	printf("come here........\n");
	return 0;
